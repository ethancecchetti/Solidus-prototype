...
public class SecurityParamsBenchmark {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        int TREE_DEPTH = 16;
        int[] BUCKET_SIZE = { 2, 3 };
        int[] STASH_SIZE = IntStream.range(15, 35 + 1).toArray();
        ...
        int ORAM_ELTS = (1 << TREE_DEPTH);
        List<ECPoint> keys = _buildKeyList(ORAM_ELTS, params);

        for (int bucketSize : BUCKET_SIZE) {
            for (int stashSize : STASH_SIZE) {
                run(params, keys, secretKey, TREE_DEPTH, bucketSize, stashSize);
            }
        }
    }
